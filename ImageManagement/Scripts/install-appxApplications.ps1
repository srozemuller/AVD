function Download-AppxPackage {
    ...
    process {
        ...
        $LinksMatch = ($WebResponse.Links  | where { $_ -like '*_neutral_*' -or $_ -like "*_" + $env:PROCESSOR_ARCHITECTURE.Replace("AMD", "X").Replace("IA", "X") + "_*" } | Select-String -Pattern '(?<=a href=").+(?=" r)').matches.value
        $Files = ($WebResponse.Links | where { $_ -like '*_neutral_*' -or $_ -like "*_" + $env:PROCESSOR_ARCHITECTURE.Replace("AMD", "X").Replace("IA", "X") + "_*" } | where { $_ } | Select-String -Pattern '(?<=noreferrer">).+(?=</a>)').matches.value
...
try {
    ...
    $appInstallerFile = Get-ChildItem -Path $downloadLocation | Where { ($_.Name -match 'Microsoft.DesktopAppInstaller') -and ($_.Name.Endswith('.msixbundle')) } | Sort-Object | Select-Object -Last 1
    $appInstallerFile | Add-AppxPackage
}
catch {
    "App not installed"
    $_
